...
class TD3_BC(object):
	def __init__(
		...
		self.policy_lr_schedule = CosineAnnealingLR(self.actor_optimizer, 1e-6)
	...
	def train(self, replay_buffer, batch_size=256):
		...
		# Compute critic loss
		critic_loss = F.mse_loss(current_Q1, target_Q) + F.mse_loss(current_Q2, target_Q) + 0.01 * (current_Q1**2 + current_Q2**2).mean()
		...
		if self.total_it % self.policy_freq == 0:
			...
			actor_loss = -lmbda * Q.mean() + self.weight * F.mse_loss(pi, action) 
